...
for line in e:
    edge_info = line.split()
    HPI_Graph.add_edge(edge_info[0], edge_info[1], weight=float(edge_info[2]))
    weight_dict.update({(edge_info[0], edge_info[1]) : float(edge_info[2])})
e.close()
...
# recTNF, precTNF = precision_recall(TNF_ranked_edges, TNF_Graph)
...
# plt.plot(recTNF,precTNF, label="TNF")
...
def shortest_paths(Graph):
    ...
    sp_dict = {}
    for t in tfs:
        spaths = nx.single_source_dijkstra_path(HPI_Graph, t)
        for r in receptors:
            if r in spaths:
                path = spaths[r]
...
def RWR(Graph):
    ...
    for i in range(np.shape(A)[0]):
        ...
        if D[i,i] == 0:
            print(i)
    DA = np.transpose(np.linalg.inv(D) * A)
    ...
    q = 0.5
    p = linalg.inv(np.identity(np.shape(A)[0]) - (1-0.5)*DA) * (q*s)
